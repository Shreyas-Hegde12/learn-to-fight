...
class VillainBrain:
    ...
    def decide_action(self, villain, player):
        ...
        # =================================================================
        # --- HARD MODE ---
        # "Escape kick/punch with shield. If not, run away. If cornered, jump over.
        #  1/5 Aggression. Dodge Shoots (Jump/Shield)."
        # =================================================================
        elif self.difficulty == "Hard":
            
            # A. PROJECTILE DODGE (50% Chance)
            if player.projectile and player.projectile.active:
                proj_dist = abs(villain.rect.centerx - player.projectile.rect.centerx)
                if proj_dist < 200 and random.random() < 0.5:
                    if can_shield and random.random() < 0.5:
                        final_action = "SHIELD"
                        self.action_cooldown = 20
                    else:
                        final_action = "JUMP"
            ...
            # B. REACT TO ATTACKS
            if is_player_attacking and gap < 100:
                # 1. Try Shield first
                if can_shield: 
                    final_action = "SHIELD"
                    self.action_cooldown = 20
                ...
                # 2. No Shield? Escape!
                    ...
                    if is_cornered:
                        # "Jump to other side and keep moving"
                        # We jump, but we must set direction TOWARDS player to cross up
                        final_action = "JUMP"
                        self.state_buffer = run_towards_dir # Buffer movement for mid-air
                        self.action_cooldown = 20 
                    else:
                        final_action = run_away_dir
                        self.action_cooldown = 10

            # C. AGGRESSION (1/5 Probability to run towards)
            elif random.random() < 0.2 and gap > 80:
            ...
            elif gap > 300 and not villain.has_shot: final_action = "SHOOT"

            # E. DEFAULT MOVEMENT (Spacing)
            elif gap < 50:
                 final_action = run_away_dir
